...
[Serializable]
public class Question //問題作成クラス
{
    //public string Furigana;
    public string japanese;
    public string roman;
}
// public class QuestionTexts //テキストから読み取るクラス
// {
...
public class TypingManager : MonoBehaviour
{
    ...
    //[SerializeField] private TextMeshProUGUI textF;//ふりがな
    [SerializeField] private TextMeshProUGUI textQ;//問題
    [SerializeField] private TextMeshProUGUI textR;//入力
    [SerializeField] public TextMeshProUGUI CountA;//問題カウント
    [SerializeField] private HPGage hpGage;//プレイヤーHP 
    [SerializeField] private Enemysprite Enemy;//敵ランダム出現
    [SerializeField] AudioSource hitSE;//成功
    [SerializeField] AudioSource missSE;//失敗

    private readonly List<char> _roman = new List<char>();//１文字区切り管理リスト
    private int _romanIndex;//現在位置（タイプ）
    private int count = 0;//クリアカウント
    private int clearconut = 15; //クリアカウント上限
    private bool _isWindows;//端末
    private bool _isMac;
    ...
    private void Start()
    {
        ...
        InitializeQuestion();
        //デバイス認識
        if (SystemInfo.operatingSystem.Contains("Windows")) { _isWindows = true; }
        if (SystemInfo.operatingSystem.Contains("Mac")) { _isMac = true; }
    }
    ...
    private void OnGUI()
    {
        ...
        {
            /* 
            現在のキーコードを取得(Event.current.kecode)
            ストリング型に変換するカスタム関数(GetCharFromKeyCode())
            文字をInputKeyメソッドに渡す(InputKey())      
            */
            switch (InputKey(GetCharFromKeyCode(Event.current.keyCode)))
            {
                case 1: // 正解タイプ時
                    ...
                    if (_roman[_romanIndex] == '@')
                    {
                        ...
                        count++;
                        CountA.text = "ターン" + count;
                        if (count == clearconut)    //戦闘終了
                        {
                            Time.timeScale = 0f;
                            this.gameObject.SetActive(false);
                            Debug.Log("戦闘終了:スコア"+ score);
                        }
                    }
                    else
                    {
                        // '@'でない場合、ローマ字テキストを更新する
                        textR.text = GeneratetextR();
                    }
    ...
    void InitializeQuestion() // 質問をランダムに選び、初期化する
    {
        ...
        //textF.text = question.Furigana; // ふりがなのテキスト表示
        textQ.text = question.japanese; // 日本語のテキストを表示
        textR.text = GeneratetextR(); // ローマ字のテキストを表示
    }

    string GeneratetextR() // ローマ字のテキストを生成する
    {
